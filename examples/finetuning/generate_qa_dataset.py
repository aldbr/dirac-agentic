"""Generate Q&A pairs from documentation using a local LLM.

Reads text files from a directory, chunks them, and generates
question-answer pairs using a quantized Qwen model. Supports
...
# 4.  Chunk generator
# ------------------------------------------------------------------------------
def iter_chunks(folder: Path, max_tokens: int, tokenizer: AutoTokenizer) -> Iterator[str]:
...
def main() -> None:
    ...
    for batch in tqdm(
        ...
        for raw in outs:
            try:
                collected.extend(q.model_dump() for q in extract_qas(raw[0]["generated_text"]))
            except Exception as e:
                print(f"bad JSON: {e}\n", raw)
        OUT_FILE.write_text(json.dumps(collected, indent=2, ensure_ascii=False))

    print(f"finished -- total {len(collected)} QAs saved to {OUT_FILE}")
